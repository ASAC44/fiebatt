...
through a local scratch directory (settings.storage_path). In S3 mode:
  • save_upload/write_bytes/publish upload the scratch file to S3
...
URLs so offline development still works.
"""
...
def download_url_for_key(key: str, *, filename: str | None = None) -> str:
    ...
    signed GET params. S3 will echo that as a `Content-Disposition: attachment` header
...
async def publish(path: Path, *, content_type: Optional[str] = None) -> str:
    """Upload a local scratch file to S3. Returns the external URL.
    ...
    log.debug("published %s -> s3://%s/%s", path, settings.s3_bucket, key)
    return url_for_key(key)
...
async def _download_to(key: str, dest: Path) -> None:
    ...
    await asyncio.to_thread(c.download_file, settings.s3_bucket, key, str(dest))
