"""CLIP embedding service backed by the vision worker.
...
async def embed_image(image_path: str) -> list[float]:
    """Get a CLIP embedding for a single image via the vision worker.
...
async def batch_embed_images(image_paths: list[str]) -> list[list[float]]:
    """Get CLIP embeddings for a batch of images via the vision worker.
