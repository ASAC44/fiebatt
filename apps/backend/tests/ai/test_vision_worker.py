"""Vision-worker integration tests for SAM and CLIP."""

import os
import pytest

pytestmark = pytest.mark.skipif(
    not (os.getenv("VISION_WORKER_URL") or os.getenv("GPU_WORKER_URL")),
    reason="VISION_WORKER_URL not set",
)


@pytest.mark.asyncio
async def test_vision_worker_health():
    from app.ai.services.sam import is_available
...
async def test_sam_segmentation(tmp_path):
    ...
    import subprocess
    from app.ai.services.sam import bbox_to_mask
...
async def test_clip_similarity():
    ...
    from app.ai.services.clip_search import cosine_similarity, embed_image
    import subprocess
    from pathlib import Path
    import tempfile
